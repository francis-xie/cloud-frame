...
public class SecureUtil {
	...
	private static byte[] sha1(byte[] data) {
		...
		try {
			...
			SDKConfig.getConfig().getLogger().error("SHA1计算失败", e);
			return null;
		}
	}
	...
	private static byte[] sha256(byte[] data) {
		...
		try {
			...
			SDKConfig.getConfig().getLogger().error("SHA256计算失败", e);
			return null;
		}
	}
	...
	private static byte[] sm3(String datas, String encoding) {
		try {
			...
			SDKConfig.getConfig().getLogger().error("SM3计算失败", e);
			return null;
		}
	}
	...
	private static byte[] decryptData(PrivateKey privateKey, byte[] data)
		...
		try {
			...
			SDKConfig.getConfig().getLogger().error("解密失败", e);
		}
		return null;
	}
