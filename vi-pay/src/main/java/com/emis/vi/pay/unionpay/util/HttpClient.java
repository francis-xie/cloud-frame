...
import com.emis.vi.pay.unionpay.util.BaseHttpSSLSocketFactory.TrustAnyHostnameVerifier;
/**
 * 
...
public class HttpClient {
	...
	public int send(Map<String, String> data, String encoding) throws Exception {
		try {
			...
			SDKConfig.getConfig().getLogger().info("请求报文:[" + sendData + "]");
			this.requestServer(httpURLConnection, sendData,
					encoding);
			this.result = this.response(httpURLConnection, encoding);
			SDKConfig.getConfig().getLogger().info("Response message:[" + result + "]");
	...
	public int sendGet(String encoding) throws Exception {
		try {
			...
			this.result = this.response(httpURLConnection, encoding);
			SDKConfig.getConfig().getLogger().info("同步返回报文:[" + result + "]");
			return httpURLConnection.getResponseCode();
		} catch (Exception e) {
			throw e;
		}
	...
	private String response(final HttpURLConnection connection, String encoding)
		...
		try {
			...
			SDKConfig.getConfig().getLogger().info("HTTP Return Status-Code:["
	...
	private String getRequestParamString(Map<String, String> requestParam, String coder) {
		...
		SDKConfig.getConfig().getLogger().info("Request Message:[" + reqstr + "]");
		return reqstr;
	}
